...
function Pagination({ currentPage = 1, onPageClick, maxPages = 8,
  ...
  onNextClick, onPreviousClick,className }) {
  const pages = pagination(totalPages, maxPages, currentPage)

  return (
    <div className={`flex  gap-4  h-12 w-min  select-none ${className}`}>
      ...
      {pages.map(page => <span onClick={page !== '..' && (() => onPageClick(page))}
        className={`text-gray-600 text-sm hover:text-green-500  ${currentPage === page && 'bg-green-500 border-0 text-white hover:text-white'} w-9 cursor-pointer inline-flex  items-center justify-center rounded font-semibold border`}
        key={page}>{page}</span>)}
