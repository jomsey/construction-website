...
function Pagination({ currentPage = 1, onPageClick, maxPages = 8,
  ...
  return (
    <div className='flex  gap-4  h-12 w-min p-2'>
      {currentPage > 1 && usePageControls && <>{currentPage > maxPages &&
        <span className='control text-green-500 cursor-pointer inline-flex  items-center justify-center  text-sm border rounded px-2'>first</span>}
        <span className='py-0 px-2 cursor-pointer  text-green-500 font-semibold border inline-flex  items-center rounded justify-centerr text-sm'
          onClick={onPreviousClick}>Previous</span></>}
      {pages.map(page => <span onClick={page !== '..' && (() => onPageClick(page))}
        className={`text-gray-700 hover:text-green-500  ${currentPage === page && 'bg-green-500 border-0 text-white hover:text-white'} w-9 cursor-pointer inline-flex  items-center justify-center rounded font-semibold border`}
        key={page}>{page}</span>)}
